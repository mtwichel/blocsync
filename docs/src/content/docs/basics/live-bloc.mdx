...
To use `LiveBloc`, first, simply extend it instead of `Bloc`. Next, add the
`LiveEvent` mixin to your event class.
...
:::note[] There is no `LiveCubit`, only `LiveBloc`. This is because `LiveBloc`
